...
import com.example.minesweepergame.databinding.ActivityGamePlayBinding
...
class GamePlayActivity : AppCompatActivity() {
    ...
    var fastestTime = " NA"
    var lastGameTime = " NA"
    var status  = com.example.minesweepergame.Status.ONGOING
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!isPlay){
            binding.chmeterGameTimer.base = SystemClock.elapsedRealtime()
            binding.chmeterGameTimer.start()
        ...
        // Restarting the game
        binding.imgBtnRestart.setOnClickListener{
            gameRestart()
        }
    }
    ...
    private fun setUpBoard(row: Int, col: Int, mine: Int) {
        ...
        binding.tvMinesCount.text = "" +mine

        // Array of buttons to find the position of a particular button
        val cellBoard = Array(row) { Array(col) {MineCell(this)}}

        binding.imgBtnMine.setOnClickListener{
            if(choice==1) {
                binding.imgBtnMine.setImageResource(R.drawable.flag)
                choice=2
            }else{
                binding.imgBtnMine.setImageResource(R.drawable.bomb)
                choice=1
            }
        }
        var counter = 1
...
//        setting up the linear layout
        for(i in 0 until row){
            ...
            binding.gameBoard.addView(linearLayout)
        }
    }
...
//    function to set random mines when user first clicks on the board
    ...
    private fun startTimer(){
        chronometer = findViewById(R.id.chmeter_GameTimer)
    ...
    private fun move(choice: Int, x: Int, y:Int, cellBoard:Array<Array<MineCell>>, rowSize: Int,colSize: Int,mine:Int): Boolean{
        ...
        if(choice==1){
            ...
            if(cellBoard[x][y].value == MINE){
                status = com.example.minesweepergame.Status.LOST;
                updateScore()
                return true
            }
        ...
        if(choice == 2){
            ...
            binding.tvMinesCount.text = ""+finalMineCount
            return true;
        }
    ...
    //Defining the array of handling 8 direction matrices
    ...
    private fun checkStatus(cellBoard:Array<Array<MineCell>>, rowSize:Int, colSize: Int){
        ...
        if(flag1==0 || flag2==0) status = com.example.minesweepergame.Status.WON
        else status = com.example.minesweepergame.Status.ONGOING

        if(status == com.example.minesweepergame.Status.WON) updateScore()
    ...
    private fun updateScore(){
        ...
        if(status == com.example.minesweepergame.Status.WON) {
...
//        if user lost then sending the result to result activity
        if(status == com.example.minesweepergame.Status.WON){
            ...
            var currentTime = binding.chmeterGameTimer.text.toString()
...
//        if user win then seding the result to main activity
        else if(status == com.example.minesweepergame.Status.LOST){
...
//            binding.chmeterGameTimer.stop()
    ...
    private fun toMainActivity(){
        ...
        val intent = Intent(this@GamePlayActivity,MainActivity::class.java)
    ...
    private fun display(cellBoard:Array<Array<MineCell>>) {
        cellBoard.forEach { row ->
            row.forEach {
                ...
                else if (status == com.example.minesweepergame.Status.LOST && it.value == MINE) {
                    binding.imgBtnRestart.setImageResource(R.drawable.sad_face)
                    it.setBackgroundResource(R.drawable.mine)
                }
                //To show that mine is not present here but it is marked
                if(status == com.example.minesweepergame.Status.LOST && it.isMarked && !it.isMine){
                    it.setBackgroundResource(R.drawable.crossedflag)
                }
                else if (status == com.example.minesweepergame.Status.WON && it.value == MINE) {
                    it.setBackgroundResource(R.drawable.flag)
                    binding.imgBtnRestart.setImageResource(R.drawable.won)
                }
                else
                    it.text = " "
            }
